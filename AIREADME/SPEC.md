...
- 标题使用固定 23 pt 高的模板图片形成紧凑 2×2 摘要，图片内部以 10 pt 等宽字形和固定两列坐标绘制。左上显示 `P` 核心热点，左下显示 `E` 核心热点，右上和右下只显示动态枚举的第一、第二台风扇实际 RPM 数值；状态项省略风扇标签和单位，电池温度与完整风扇信息保留在展开面板。当前双风扇结果不得来自硬编码 SMC 索引。
