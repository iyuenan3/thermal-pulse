...
- 提供 5 分钟、15 分钟和 1 小时历史曲线，默认选择已验证风扇，用户可手动选择最多 6 项。
- RPM 与原始温度候选分图展示，显示前进行保留端点、极值和采样间隙的降采样。
- 普通逻辑测试，以及需要显式选择的当前 Mac 只读硬件探测测试。

当前不包含传感器中文语义确认、SMC 写入、privileged helper、Turbo、签名、安装包或发布版本。加速数据测试已经覆盖 312 个序列的 1 小时容量边界，真实 1 小时运行和人工界面验收仍待完成。
