...
- 当前已有可构建的原生 SwiftUI App、只读 `SMCReadAdapter`、typed sensor model、1 Hz 动态白名单采样、1 小时内存环形缓冲、菜单栏重开窗口入口、默认风扇和原始温度候选曲线，以及测试目标；尚无 helper、安装产物或发布版本。
...
- 已用 312 个序列加速模拟 1 Hz、3601 秒的数据路径并验证容量上限。优化后的最终 Release App 已完成 3605 秒持续运行，取得 60 个无间隔缺口的监督样本，未提前退出且日志为空；人工界面确认、温度语义确认和未归因性能台阶仍待完成，Turbo 尚未通过。
