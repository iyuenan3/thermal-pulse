...
- 当前已有可构建的原生 SwiftUI App、只读 `SMCReadAdapter`、typed sensor model、1 Hz 动态白名单采样、1 小时内存环形缓冲、可选传感器历史曲线和测试目标；尚无 helper、安装产物或发布版本。
- 当前 Mac16,7 已用普通权限读回 2 个风扇的实际与最大 RPM，并对 312 个动态采样项完成连续两轮零失败读回。该证据只覆盖短时只读链路，不代表长时间稳定性或 Turbo 已通过。
- 已用 312 个序列加速模拟 1 Hz、3601 秒的数据路径并验证容量上限；真实 1 小时运行、人工界面检查和温度语义确认仍待完成。
